...
async def create_book(
    ...
    new_book = users.Book(**book.model_dump())
    db.add(new_book)
    await db.commit()
...
async def get_books(db: AsyncSession = Depends(get_db)):
    books = await db.execute(select(users.Book))
...
async def get_book(id: int, db: AsyncSession = Depends(get_db)):
    book = await db.execute(select(users.Book).where(users.Book.id == id))
    book = book.scalar()
    if not book:
...
async def delete_author(
    ...
    author_book = await db.execute(
        select(users.BookAuthor)
        .where(users.BookAuthor.book_id == book.id)
        .where(users.BookAuthor.author_id == author.id)
    )
